...
## Import TVB files:
from tvb_model_reference.simulation_file.parameter.parameter_M_Berlin_dt01s import Parameter
parameters = Parameter()

import tvb_model_reference.src.nuu_tools_simulation_human as tools
#from tvb_model_reference.view.plot import multiview, multiview_one, animation_nuu
...
Iext = 0.001 # initialement 0.001 # External input # noise level
coupling_strength = 0.15
ratio_coupling_EE_EI = 1.4 #initialement 1.4 # ratio of E to I/E to E inter-regional couplng strengths 
...
Qi_default = [4., 4.2, 4.4, 4.6, 4.8, 5., 5.3, 5.5, 6.] # in nS, inhib conductance, whole brain #5.
...
for Qi in Qi_default:
    Qi_change = np.array([5.0]) #np.arange(10,100,10) #  inhibitory conductance, to change in specific region
    ...
    for Qi_change_curr in Qi_change:
        for simnum in seeds_all:
            ...
            cut_transient = 0.0 + add_transient# ms, length of the discarded initial segment # 4000 to be similar to the real dataset
            run_sim =  100 *10**3  + cut_transient# for feature attribution testing to match human fMI data length 100 s # temps de simulation en ms
            ...
            path_save = folder_root +'_b_'+str(bvals) +'_Qi_'+node_name_Qichange+str(Qi)+\
            ...
            try:
                ...
                EIsig = 0.8*Esig + 0.2*Isig # 0.8*Esig + 0.2*Isig_alltime #CHECK !!!!!!!!!!!!!!!!!!!
                

                # save file
                #np.save('sig_EI.npy', EIsig)
                ...
                if save_BOLD:
                    ...
                    for idx_reg in range(len(FR_sum[0])):
                        conv = signal.fftconvolve(EIsig[:,idx_reg],
                                                  kernel_hrf, mode = 'valid')# 'same' initialement
                        BOLD_curr.append(conv)
                   
                    """print('BOLD_curr shape before cutoff = ', np.array(BOLD_curr).shape)
                    # Calcul du cutoff basé sur la taille du kernel HRF
                    cutoff = len(kernel_hrf) - 1
                    print('cutoff = ', cutoff)

                    # Coupe avant de faire le subsampling
                    if cutoff >= 1:
                        print('Here, cutting off last', cutoff, 'samples')
                        BOLD_curr = [conv[:-cutoff] for conv in BOLD_curr]  # Enlever les effets de bord

                    BOLD_curr = np.array(BOLD_curr)
                    print('BOLD_curr shape after cutoff = ', BOLD_curr.shape)"""
                    ...
                    path_save_BOLD = '_b_'+str(bvals) +'_Qi_'+node_name_Qichange+str(Qi) \
                    +'_repeated'+stim_region_name+'stim_'+str(stimval)+\
                    'EtoEIratio'+str(ratio_coupling_EE_EI)+'_coupling'+str(coupling_strength)+\
                    'seed'+str(seed_curr)+'_noise'+str(Iext)+'/'
...
print('Total time = ', str(np.round(end-start, 3)), ' seconds')               
