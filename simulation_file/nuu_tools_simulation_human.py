...
# Récupère le dossier parent de ce fichier (simulation_file/)
current_dir = os.path.dirname(os.path.abspath(__file__))

# Ajoute le dossier parent de simulation_file (donc tvb_model_reference/)
parent_dir = os.path.abspath(os.path.join(current_dir, '..'))
sys.path.append(parent_dir)
from results.synch.parameter import parameter_tvb
...
from inference.params import default_params 
seed = default_params['seed']  # Utiliser le seed par défaut défini dans params.py
...
def init(parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
    ...
    try:
        if parameter_model['matteo']:
            import tvb_model_reference.simulation_file.Zerlaut_matteo as model # type : ignore
        else:
            import tvb_model_reference.simulation_file.Zerlaut as model
    except ModuleNotFoundError:
        try:
            if parameter_model['matteo']:
                import tvb_model_reference.simulation_file.Zerlaut_matteo as model
            else:
                import tvb_model_reference.simulation_file.Zerlaut as model
        except ModuleNotFoundError as e:
            raise ImportError(
                f"❌ Impossible de charger le module Zerlaut ou Zerlaut_matteo.\n"
                f"Vérifiez si les fichiers ont été déplacés dans simulation_file au lieu de src.\n"
                f"Erreur exacte : {e}"
            )
...
#%%
    ...
    #f = open('save_file/parameter.json',"w")
    f.write("{\n")
...
def run_simulation(simulator, time, parameter_simulation, parameter_monitor):
    ...
    for result in simulator(simulation_length=time):
        if not isinstance(result, (list, tuple)) or len(result) != nb_monitor:
            print(f" Problème : result mal formé à step {count}, type: {type(result)}, len: {len(result)}")
            continue
        ...
        for i in range(nb_monitor):
            if result[i] is not None:
                ...
                print(f"result[{i}] is None at step {count}")

        # Vérifie si on doit enregistrer un snapshot
        if result[0][0] >= parameter_simulation['save_time'] * (count + 1):

            try:
                res = np.array(save_result, dtype=object)
                res_list_numpy.append(res)
            except Exception as e:
                print(f" ERREUR création de res (step {count}) : {e}")
                print(f" Contenu save_result:\n{save_result}")
                raise

            # Reset
            save_result = [[] for _ in range(nb_monitor)]
            count += 1
    return res_list_numpy


def old_run_simulation(simulator, time, parameter_simulation,parameter_monitor):
    '''
    run a simulation
...
def get_result(res_list_numpy, path, time_begin, time_end):
    '''
    return the result of the simulation between the wanted time
    :param res_list_numpy: list of saved results (in-memory)
    :param path: the folder of the simulation (not always used here)
    :param time_begin: the start time for the result
    :param time_end:  the ending time for the result
    :return: result of all monitor
    '''
    try:
        parameters = parameter_tvb.copy()
    except Exception as e:
        print(' Error: parameter_tvb not found or inaccessible')
        raise e
    ...
    #print(f'count_begin: {count_begin}, count_end: {count_end}')
    nb_monitor = (
        parameter_monitor['Raw']
        + parameter_monitor['TemporalAverage']
        + parameter_monitor['Bold']
        + parameter_monitor['Ca']
    )
    if 'Afferent_coupling' in parameter_monitor and parameter_monitor['Afferent_coupling']:
        nb_monitor += 1
    #print(f" Nombre de moniteurs: {nb_monitor}")
    output = []

    for count, i in enumerate(range(count_begin, count_end-1)):
        
        try:
            result = res_list_numpy[i]
        except IndexError:
            print(f" res_list_numpy[{i}] est hors limites !")
            continue

        for mon_idx in range(result.shape[0]):

            tmp = np.array(result[mon_idx])
            if len(tmp) != 0:
                ...
                if tmp_time.shape[0] != 0:
                    ...
                    if len(output) == nb_monitor:
                        output[mon_idx]=[np.concatenate([output[mon_idx][0],tmp_time]),np.concatenate([output[mon_idx][1],tmp_value])]
                    else:
...
def old_get_result(path,time_begin,time_end, prints=0):
    '''
    return the result of the simulation between the wanted time
